...
use proto::funder::messages::{AddFriend, SetFriendRelays, SetFriendRemoteMaxDebt,
                                ResetFriendChannel};
    ...
    R: CryptoRandom,
{
    ...
    pub async fn add_relay(&mut self, named_relay_address: NamedRelayAddress) -> Result<(), AppConfigError> {
        await!(self.send_request(AppRequest::AddRelay(named_relay_address)))
    }

    pub async fn remove_relay(&mut self, relay_public_key: PublicKey) -> Result<(), AppConfigError> {
        await!(self.send_request(AppRequest::RemoveRelay(relay_public_key)))
    }

    pub async fn add_friend(&mut self, friend_public_key: PublicKey,
                            relays: Vec<RelayAddress>,
                            name: String,
                            balance: i128) -> Result<(), AppConfigError> {
    ...
    pub async fn set_friend_relays(&mut self, 
                                   friend_public_key: PublicKey, 
                                   relays: Vec<RelayAddress>) -> Result<(), AppConfigError> {
    ...
    pub async fn remove_friend(&mut self, friend_public_key: PublicKey) -> Result<(), AppConfigError> {
        await!(self.send_request(AppRequest::RemoveFriend(friend_public_key)))
    }

    pub async fn enable_friend(&mut self, friend_public_key: PublicKey) -> Result<(), AppConfigError> {
        await!(self.send_request(AppRequest::EnableFriend(friend_public_key)))
    }

    pub async fn disable_friend(&mut self, friend_public_key: PublicKey) -> Result<(), AppConfigError> {
        await!(self.send_request(AppRequest::DisableFriend(friend_public_key)))
    }

    pub async fn open_friend(&mut self, friend_public_key: PublicKey) -> Result<(), AppConfigError> {
        await!(self.send_request(AppRequest::OpenFriend(friend_public_key)))
    }

    pub async fn close_friend(&mut self, friend_public_key: PublicKey) -> Result<(), AppConfigError> {
        await!(self.send_request(AppRequest::CloseFriend(friend_public_key)))
    }

    pub async fn set_friend_remote_max_debt(&mut self, friend_public_key: PublicKey,
                                            remote_max_debt: u128) -> Result<(), AppConfigError> {

        let set_friend_remote_max_debt = SetFriendRemoteMaxDebt {
            friend_public_key,
            remote_max_debt,
        };
        await!(self.send_request(AppRequest::SetFriendRemoteMaxDebt(set_friend_remote_max_debt)))
    }

    pub async fn reset_friend_channel(&mut self, friend_public_key: PublicKey,
                                      reset_token: Signature) -> Result<(), AppConfigError> {
    ...
    pub async fn add_index_server(&mut self, named_index_server: NamedIndexServerAddress) -> Result<(), AppConfigError> {
        await!(self.send_request(AppRequest::AddIndexServer(named_index_server)))
    }

    pub async fn remove_index_server(&mut self, index_public_key: PublicKey) -> Result<(), AppConfigError> {
