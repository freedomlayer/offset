...
mod tests {
    ...
    use proto::index_client::messages::{IndexClientReport, IndexClientReportMutation, 
        ClientResponseRoutes, ResponseRoutesResult};
    use proto::index_server::messages::RequestRoutes;
    use proto::funder::messages::{UserRequestSendFunds, FriendsRoute, 
        InvoiceId, INVOICE_ID_LEN, ResponseReceived, ResponseSendFundsResult};

    /// A test util function.
    /// Spawns an app server loop and returns all relevant channels
    /// used for control or communication.
    fn spawn_dummy_app_server<S>(mut spawner: S) -> 
