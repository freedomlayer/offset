...
impl FutTransform for Resolver {
    ...
    fn transform(&mut self, net_address: Self::Input)
        ...
        let resolve_fut = future::lazy(move |_| {
            if let Ok(socket_addr_iter) = (&net_address.0).to_socket_addrs() {
...
mod tests {
    ...
    fn test_resolver_numeric_v4() {
        ...
        let res_vec = thread_pool.run(resolver.transform("127.0.0.1:1337".to_owned().into()));
    ...
    fn test_resolver_numeric_v6() {
        let mut thread_pool = ThreadPool::new().unwrap();
        let mut resolver = Resolver::new().unwrap();
        let res_vec = thread_pool.run(resolver.transform("::1:1338".to_owned().into()));
    ...
    fn test_resolver_localhost_v4() {
        let mut thread_pool = ThreadPool::new().unwrap();
        let mut resolver = Resolver::new().unwrap();
        let res_vec = thread_pool.run(resolver.transform("localhost:1339".to_owned().into()));
