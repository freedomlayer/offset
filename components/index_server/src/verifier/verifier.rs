...
pub trait Verifier {
    ...
    fn verify(&mut self, 
                   ...
                   counter: u64) -> Option<Vec<HashResult>>; 
                    // TODO: Can we change the return value to be a slice?
    ...
    fn tick(&mut self) -> (HashResult, Vec<Self::Node>);
                            // TODO: Can we change to &HashResult?

    /// Process a time tick from a neighbor. This information is used when producing a `tick_hash`.
    fn neighbor_tick(&mut self, neighbor: Self::Neighbor, tick_hash: HashResult) -> Option<HashResult>;
