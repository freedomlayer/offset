...
pub async fn compact_node_init<CG>(
    ...
    compact_gen: &mut CG) -> Result<(), CompactNodeError>
where   
    CG: GenUid,
{
    ...
    for payment_id in &payment_ids {
        ...
        match &open_payment.status {
            ...
            | OpenPaymentStatus::Sending(_) => {
                ...
                conn_pair_compact.sender.send(CompactToUserAck::CompactToUser(compact_to_user)).await.map_err(|_| CompactNodeError::UserSenderError)?;
            },
