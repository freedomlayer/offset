...
use crate::compact_node::types::{ConnPairCompact, CompactNodeError};
...
pub async fn compact_node<CG>(
    ...
    mut compact_gen: CG) -> Result<(), CompactNodeError>
where
    CG: GenUid
{
    compact_node_init(&mut conn_pair_compact, &mut compact_state, &mut database_client, &mut compact_gen).await?;
    compact_node_loop(app_conn_tuple, conn_pair_compact, compact_state, database_client, compact_gen).await
}
