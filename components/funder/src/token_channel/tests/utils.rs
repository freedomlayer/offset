        ...
        Box::pin(async move {
            match self.status {
                MockTcStatus::Consistent(tc_consistent) => match tc_consistent.direction {
                    MockTcDirection::In(move_token_in) => TcStatus::ConsistentIn(move_token_in),
                    MockTcDirection::Out(move_token_out, opt_move_token_in) => {
                        TcStatus::ConsistentOut(move_token_out, opt_move_token_out)
                    }
                },
                MockTcStatus::Inconsistent(local_reset_terms, opt_remote_reset_terms) => {
                    TcStatus::Inconsistent(
                        ...
                        opt_remote_reset_terms.map(|remote_reset_terms| {
                            (
                                remote_reset_terms.reset_token,
                                remote_reset_terms.reset_move_token_counter,
                            )
                        }),
                    )
                }
