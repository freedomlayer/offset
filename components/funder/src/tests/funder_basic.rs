...
async fn task_funder_basic(test_executor: TestExecutor) {
    ...
    let pred = |report: &FunderReport<_>| {
        ...
        let opt_pos = tc_report.currency_reports.iter().position(|currency_report| currency_report.currency == currency1);
        if let Some(pos) = opt_pos {
            dbg!(tc_report.currency_reports[pos].balance.balance) == -5
        } else {
            false
        }
    };
    ...
    let pred = |report: &FunderReport<_>| {
        ...
        let opt_pos = tc_report.currency_reports.iter().position(|currency_report| currency_report.currency == currency1);
        if let Some(pos) = opt_pos {
            tc_report.currency_reports[pos].balance.balance == 5
        } else {
            false
        }
    };
