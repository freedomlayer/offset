...
use app::common::{PublicKey, PaymentStatus, Currency, MultiRoute, PaymentId, Uid, FriendsRoute};
use app::conn::{ConnPairApp, AppToAppServer, AppServerToApp, AppRequest, self, ResponseRoutesResult, RequestResult};
use app::gen::gen_uid;
...
pub async fn send_request(conn_pair: &mut ConnPairApp, app_request: AppRequest) -> Result<(), AppWrapperError> {
    ...
    conn_pair.sender.send(app_to_app_server).await.map_err(|_| AppWrapperError);
...
pub async fn request_routes(conn_pair: &mut ConnPairApp, currency: Currency, dest_payment: u128, src_public_key: PublicKey,
...
pub async fn request_close_payment_nowait(conn_pair: &mut ConnPairApp, payment_id: PaymentId) -> Result<(), AppWrapperError> {
...
pub async fn request_close_payment(conn_pair: &mut ConnPairApp, payment_id: PaymentId) -> Result<PaymentStatus, AppWrapperError> {
...
/// Request to close payment, but do not wait for the payment to be closed.
pub async fn ack_close_payment(conn_pair: &mut ConnPairApp, payment_id: PaymentId, ack_uid: Uid) -> Result<(), AppWrapperError> {
...
pub async fn create_transaction(conn_pair: &mut ConnPairApp, 
    payment_id: PaymentId,
    request_id: Uid,
    route: FriendsRoute,
    dest_payment: u128,
    fees: u128) -> Result<RequestResult, AppWrapperError> {

    let app_request = conn::buyer::create_transaction(payment_id.clone(), request_id.clone(), route, dest_payment, fees);
    let app_request_id = gen_uid();
    let app_to_app_server = AppToAppServer {
        // We don't really care about app_request_id here, as we can wait on `request_id`
        // instead.
