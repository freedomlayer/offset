...
Note that the event of B handing the confirmation to E is atomic. The moment
...
buyer and before the Commit message was sent.
...
In the figure above: a request was sent from B to E. Next, D unfriends E before
...
value and hashing it: `srcHashedLock := sha512/256(srcPlainLock)`. This
mechanism is used to ensure transaction atomicity: The seller
(seller) can not create a valid Commit message without knowing the secret value `srcPlainLock`.
...
sha512/256(destPlainLock)`. This secret will only be revealed when the Commit
...
After Request message arrived at the seller node and before the Commit
...
in case of unfriending a node (As long as the Commit message was not yet
received).

After the Commit message was received, the transaction can not be cancelled.
...
gives the goods to the buyer and sends back a Commit message to
collect his credits.
...
Note that the Commit message can only be sent by the seller after
...
- If `ResponsePay` contains cancel, cancel the payment.
  Else, send Confirmation to the seller.
...
- Send an invoice to the buyer (Out of band), and wait for Confirmation from
    the buyer.
