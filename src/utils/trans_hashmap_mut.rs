...
pub struct TransHashMapMut<'a, K: 'a, V: 'a> {
    hmap: &'a mut HashMap<K,V>,
    orig: HashMap<K,Option<V>>,
}

impl<'a, K,V> TransHashMapMut<'a, K,V>
where
    K: Eq + Hash + Clone,
    V: Clone,
{
    /// Create from an existing hash map.
    pub fn new(hmap: &'a mut HashMap<K,V>) -> Self {
        TransHashMapMut {
            hmap,
            orig: HashMap::new(),
        }
    ...
    pub fn cancel(self)  {
        let TransHashMapMut {mut hmap, orig} = self;
...
mod tests {
    ...
    fn test_trans_hash_map_cancel() {
        ...
        let mut tmap = TransHashMapMut::new(&mut hmap);
