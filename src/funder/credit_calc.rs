...
use super::types::{NetworkerFreezeLink, PaymentProposalPair, FriendRouteLink, FriendsRoute};
...
fn calc_request_len(request_content_len: u32, 
    ...
    let friend_route_link_len = usize_to_u32(mem::size_of::<FriendRouteLink>())?;

    let route_bytes_count = public_key_len.checked_mul(2)?
        .checked_add(route_len.checked_sub(2)?.checked_mul(friend_route_link_len)?)?
...
mod tests {
    ...
    fn test_calc_request_len_basic() {
        ...
        */
        ...
        let friends_route_len = 
            2 * mem::size_of::<PublicKey>()
            /* +  0 * mem::size_of::<FriendRouteLink>() */
