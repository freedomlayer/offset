...
# else an attacker could consume all its memory by flooding the responder with
...
struct RespondNonce {
    ...
    responderRandNonce @2: CustomUInt128;
    signature          @3: CustomUInt512;
}

# Upon receiving a RespondNonce message, the initiator finish the following
# validations first:
# 
# 1. Check whether it sent this reqRandNonce and that nonce still alive.
# 2. Verifies the message's signature was generated by the chosen neighbor.
#
# If all validations passed, initiator generates a new nonce, DH private key,
# and key salt, keeps these information and prepare the ExchangeActive message
# as response.
#
# Implementation note: The initiator creates a new handshake session after it
# sends ExchangeActive message.
struct ExchangeActive {
    responderRandNonce  @0: CustomUInt128;
    initiatorRandNonce  @1: CustomUInt128;

    initiatorPublicKey  @2: CustomUInt256;
    responderPublicKey  @3: CustomUInt256;
    dhPublicKey         @4: CustomUInt256;
    keySalt             @5: CustomUInt256;
    signature           @6: CustomUInt512;
}

# Upon receiving ExchangeActive message, the responder first checks whether an
# ongoing handshake session with the initiator already exists. If one exists,
