...
//         );
        ...
        // A -> B: RequestNonce
        let request_nonce_to_b = handshake_client_a.initiate_handshake(public_key_b.clone()).unwrap();

        // B -> A: RespondNonce
        let mut respond_nonce_to_a = handshake_server_b.handle_request_nonce(
            request_nonce_to_b
        ).unwrap();
        ...
        // A -> B: ExchangeActive
        let mut exchange_active_to_b = handshake_client_a.handle_response_nonce(
            respond_nonce_to_a
        ).unwrap();
        ...
        // B -> A: ExchangePassive
        let mut exchange_passive_to_a = handshake_server_b.handle_exchange_active(
        ...
        let (new_channel_info_a, mut channel_ready_to_b) = handshake_client_a.handle_exchange_passive(
        ...
        // B: Finish
        let new_channel_info_b = handshake_server_b.handle_channel_ready(
            channel_ready_to_b
        ).unwrap();
