...
use channeler::config::{REQUEST_NONCE_TIMEOUT, HANDSHAKE_SESSION_TIMEOUT};
...
impl HandshakeClientSession {
    ...
    fn finish(self, recv_key_salt: Salt, remote_dh_public_key: DhPublicKey) -> Result<ChannelMetadata, HandshakeError> {
        unimplemented!()
    }
}

impl<SR: SecureRandom> HandshakeClient<SR> {
    pub fn new(local_public_key: PublicKey, neighbors: Rc<RefCell<NeighborTable>>, rng: Rc<SR>) -> HandshakeClient<SR> {
        HandshakeClient {
            local_public_key,
            neighbors,
            secure_rng: rng,
    ...
    fn allow_initiate_handshake(&self, remote_public_key: &PublicKey) -> Result<(), HandshakeError> {
        ...
        let requesting_nonce = self.request_nonce_sessions.values()
            .any(|s| *s.remote_public_key() == *remote_public_key);

        if requesting_nonce || self.public_key_to_hash_result.contains_key(remote_public_key) {
    ...
    fn check_response_nonce(&self, response_nonce: &ResponseNonce) -> Result<(), HandshakeError> {
        ...
        if verify_signature(&response_nonce.as_bytes(), remote_public_key, &response_nonce.signature) {
            Ok(())
        } else {
    ...
    pub fn handle_response_nonce(&mut self, response_nonce: ResponseNonce) -> Result<ExchangeActive, HandshakeError> {
        ...
        let local_dh_private_key = DhPrivateKey::new(&*self.secure_rng).map_err(HandshakeError::CryptoError)?;
        let local_dh_public_key = local_dh_private_key.compute_public_key().map_err(HandshakeError::CryptoError)?;
        ...
        match self.handshake_client_sessions.insert(last_hash.clone(), new_session) {
            None => {
            ...
            Some(_) => Err(HandshakeError::MessageHashCollision)
        }
    }

    fn check_exchange_passive(&self, exchange_passive: &ExchangePassive) -> Result<(), HandshakeError> {
        let remote_public_key_ref = self.handshake_client_sessions
    ...
    pub fn handle_exchange_passive(&mut self, exchange_passive: ExchangePassive) -> Result<(ChannelMetadata, ChannelReady), HandshakeError> {
        ...
        let session = self.handshake_client_sessions
            .remove(&exchange_passive.prev_hash)
            .expect("invalid exchange passive message");
        self.public_key_to_hash_result.remove(session.remote_public_key());
        ...
        let channel_metadata = session.finish(exchange_passive.key_salt, exchange_passive.dh_public_key)?;
