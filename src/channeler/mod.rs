...
//    SecurityModuleClient(SecurityModuleClientError),
//
...
//                            .map_err(ChannelerError::Channel)?;
//
...
//
//        let start_new_handshake_task = self.handshaker
//            .initiate_handshake(remote_public_key)
//            .into_future()
//            .map_err(|_e| ChannelerError::HandshakerError)
...
//        let external_sender = self.external_sender.clone();
//
//        let process_request_nonce_task = self.handshaker
...
//                    .map_err(ChannelerError::SecurityModuleClient)
...
//        let external_sender = self.external_sender.clone();
//
//        let process_respond_nonce_task = self.handshaker
...
//
//        match self.handshaker.handle_channel_ready(channel_ready) {
//            Ok(handshake_res) => {
...
//        let external_sender = self.external_sender.clone();
//
//        let process_exchange_active_task = self.handshaker
...
//        let external_sender = self.external_sender.clone();
//
//        let process_exchange_passive_task = self.handshaker
